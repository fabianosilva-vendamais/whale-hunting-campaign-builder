...
const FEED = 'REGRA DE FEED (obrigatória): todo post/peça precisa PARAR o feed. A primeira linha decide tudo — o LinkedIn corta o texto após ~2 linhas, então o gancho precisa funcionar sozinho, sem contexto. Proibido abrir com "Estamos lançando", "Você sabia que", "O cenário atual", "É com satisfação", anúncio institucional ou pergunta genérica. Abra com: um número específico e desconfortável do material, uma afirmação contraintuitiva, ou uma frase que o decisor pensa mas não diz em voz alta. Corpo em linhas curtas (1–2 frases por parágrafo, com linha em branco entre elas), uma única ideia por peça, e o post AFIRMA uma posição — nunca "vamos discutir". CTA concreto no final. Sugestões de imagem devem ser específicas e executáveis (o que aparece, qual número em destaque, qual contraste) — nunca "imagem que ilustre X".';
...
function Conteudo({ pieces, setPieces, loading, ctx, gen, savePiece, flash }) {
  ...
  const defs = {
    posts: {
      ...
      prompt: `${ctx()}\n\n${ACIONAVEL}\n\n${FEED}\n\nGere 4 POSTS ORGÂNICOS para LinkedIn. Use EXATAMENTE estes marcadores:\n=== POST 1 — Lançamento do Special Report ===\n=== POST 2 — Dado forte do setor ===\n=== POST 3 — Provocação executiva ===\n=== POST 4 — Conexão com treinamento ===\nEm CADA post use EXATAMENTE estes rótulos, um por linha:\nObjetivo:\nPúblico:\nGancho inicial:\nCopy completa:\nCTA:\nSugestão de imagem:\nFormato recomendado:\nObservação para agência:\nVersão curta:\nVersão alternativa (mais executiva):\nPOST 2 deve abrir com um dado real do relatório (o número na primeira linha) e entregar uma leitura não óbvia dele. POST 3 abre com a provocação direta (ex.: "sua cooperativa cresce em base ou em principalidade?") e o corpo SUSTENTA a tese com argumento, não repete a pergunta. POST 4 conecta os desafios à necessidade de desenvolver a equipe comercial partindo de um erro comum de execução — a solução aparece no final como caminho, nunca como anúncio.`
    },
    briefing: {
